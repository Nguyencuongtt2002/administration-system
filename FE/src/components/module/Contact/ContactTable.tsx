...
import { FormContact } from '@/components/module/Contact'
import { useSelector } from 'react-redux'
import { TFunction } from 'i18next'
import { TranslationEnum } from '@/utils/enum/common'
import { IListContact } from '@/utils/interface/contact'
import { deleteContact, getListContact } from '@/thunks/contact/contactThunk'
import { contactActions, selectIsRefreshContactList } from '@/thunks/contact/contactSlice'
import { formatPhoneNumber } from '@/utils/helper/common'
...
const ContactTableContext = createContext({
...
export const columns = (t: TFunction<TranslationEnum.TRANSLATION>): ColumnDef<TableItem>[] => [
  {
    accessorKey: 'MaLienHe',
    header: t('contact_page_table_id'),
    cell: ({ row }) => <div className='capitalize'>{row.getValue('MaLienHe')}</div>
  },
  {
    accessorKey: 'Email',
    header: t('contact_page_table_email'),
    cell: ({ row }) => <div className='capitalize'>{row.getValue('Email')}</div>
  },
  {
    accessorKey: 'DiaChi',
    header: t('contact_page_table_address'),
    cell: ({ row }) => <div className='capitalize'>{row.getValue('DiaChi')}</div>
  },
  {
    accessorKey: 'SoDienThoai',
    header: t('contact_page_table_phone'),
    cell: ({ row }) => {
      const rawValue = row.getValue<string>('SoDienThoai')
      const formatted = formatPhoneNumber(rawValue ?? EMPTY_STRING)
  ...
  },
  {
    ...
    enableHiding: false,
    cell: ({ row }) => {
      const { setTableIdEdit, setTableIdDelete, setIsOpen } = useContext(ContactTableContext)
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align='end'>
            ...
            <DropdownMenuItem
              onClick={() => {
                setTableIdEdit(row.original.MaLienHe)
                setIsOpen(true)
              }}
            >
              Sửa
            ...
            <DropdownMenuItem
              onClick={() => {
                setTableIdDelete(row.original.MaLienHe)
              }}
            >
              Xóa
            </DropdownMenuItem>
...
export default function ContactTable() {
  ...
  const isRefresh = useSelector(selectIsRefreshContactList)
  ...
  useEffect(() => {
    if (isRefresh) {
      getList(paramObject)
      dispatch(contactActions.setRefreshList(false))
    }
  }, [isRefresh])

  const getList = (payload: IQueryBase) => {
    loadingContext?.show()
    dispatch(getListContact(payload))
      .unwrap()
      .then((res) => {
        const { result, meta }: IListDataResponse<IListContact[]> = res?.data
        setData(result)
  ...
  const handleDelete = () => {
    ...
    dispatch(deleteContact(tableIdDelete))
      .unwrap()
      .then(() => {
    ...
    >
      <div className='w-full'>
        <div className='flex items-center py-4'>
          <Input
            placeholder={t('contact_page_placeholder_input')}
            value={filterName}
        ...
        <div className='flex items-center justify-end space-x-2 py-4'>
          ...
          <div>
            {table.getRowModel().rows.length > 0 && (
              <AutoPagination pageSize={table.getPageCount()} pathname='/admin/contact' />
            )}
          </div>
        </div>
      </div>

      <FormContact id={tableIdEdit} setId={setTableIdEdit} isOpen={isOpen} onClose={handleToggleForm} />
